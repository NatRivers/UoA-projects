...
import flix.authentication.services as auth_services
...
def test_comment(client, auth):
    ...
    response = client.get('/comment?movie=%3CMovie+21%2C+2008%3E')

    response = client.post(
        '/comment',
        data={'comment': 'Who watch movies?', 'movie_title': '21'}
    )
    assert response.headers['Location'] == 'http://localhost/authentication/login'
...
def test_comment_with_invalid_input(client, auth, comment, messages):
    ...
    response = client.post(
        '/comment',
        data={'comment': comment, 'movie_id': '21'}
    )
...
def test_movies_without_genre(client):
...
def test_movies_with_genre(client):
    # Check that we can retrieve the articles page.
    response = client.get('/movies_by_genre?genre=Musical')
    assert response.status_code == 200

    # Check that all articles on the requested date are included on the page.
    assert b'Across the Universe' in response.data


def test_movies_with_comment(client):
    # Check that we can retrieve the articles page.
    response = client.get('/movies?view_comments_for=%3CMovie+21%2C+2008%3E&cursor=0')
    assert response.status_code == 200

    # Check that all comments for specified article are included on the page.
    assert b'Reviews for Movie: 21' in response.data